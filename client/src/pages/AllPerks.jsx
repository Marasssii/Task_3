...
export default function AllPerks() {
  ...
  // Track first render to avoid a double fetch from the two effects
  const didInit = useRef(false)

  // ==================== SIDE EFFECTS WITH useEffect HOOK ====================

  // Derive unique merchant list from current perks
  useEffect(() => {
  ...
  // useEffect Hook #1: Initial Data Loading (load all perks once on mount)
  useEffect(() => {
    if (!didInit.current) {
  ...
  useEffect(() => {
    // Debounce to avoid firing a request on every keystroke immediately
    const t = setTimeout(() => {
      // Only run after the initial load has occurred
      if (didInit.current) {
        loadAllPerks()
      }
    }, 400)
  ...
  function handleSearch(e) {
    e.preventDefault()
    loadAllPerks() // immediate search (bypasses debounce)
  }

  function handleReset() {
    setSearchQuery('')
    setMerchantFilter('')
    // effect watching [searchQuery, merchantFilter] will reload
  }
